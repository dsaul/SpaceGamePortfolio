...
namespace SpaceInsurgency.Items
{
	public class Vulcan : Base
	{
		...
		[SerializeField]
		GameObject m_MuzzlePrefab;
		[Inspect, Group("Weapon"), DontAllowSceneObject]
		public GameObject MuzzlePrefab
		{
			get { return m_MuzzlePrefab; }
			set { m_MuzzlePrefab = value; }
		}
...
#warning toggle enabled
		...
		{
			//Debug.Log("ActivateWeaponVulcan");
			...
			obj.GetComponent<VulcanProjectile>().SpawnParent = transform;
			obj.transform.parent = transform;
			obj.transform.LookAt(target.transform);
			obj.name = string.Format("Missile - {0} - Blaster Light", sourceFaction.DisplayName);

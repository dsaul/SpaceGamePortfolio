...
#warning toggle enabled
		...
		{
			//Debug.Log("ActivateWeaponChainGun");
			...
			obj.GetComponent<SoloGunProjectile>().SpawnParent = transform;
			obj.transform.parent = transform;
			obj.transform.LookAt(target.transform);
			obj.name = string.Format("Missile - {0} - Solo Gun", sourceFaction.DisplayName);

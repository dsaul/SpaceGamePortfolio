...
{
	public class IonCannon : Base
	{
#warning this needs to be rewritten so that these objects are either instantiated or held on the ship in some way
		...
		protected override void FixedUpdate()
		{
			...
			if (null == lastActivateSource || null == lastActivateTarget || false == IsActivationAppropriate(lastActivateSource, lastActivateTarget, lastActivateSource.transform.position.SqrDistance(lastActivateTarget.transform.position)))
			{
...
#warning toggle enabled
		...
		protected override void Activate(EquippableComponent source, EquippableComponent.Entry entry, SpaceObject target, float targetSqrDistance)
		{
			...
			if (null == laserObject)
			{
				...
				laserObject = GameObject.Instantiate(AssetManager.Main.items.arcLaserCannonIon) as GameObject;
				...
				target.TakeDamage(Damage, SpaceObject.DamageType.EMP);
			}
		}

		public void OnRemovedFrom(EquippableComponent newContainer)
		{
			if (null != laserObject)
			{
				GameObject.Destroy(laserObject);
